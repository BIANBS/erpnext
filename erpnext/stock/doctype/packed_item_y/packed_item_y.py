...
class PackedItemY(Document):
	pass

def get_sales_bom_y_items(item_code):
        return frappe.db.sql("""select t1.item_code, t1.qty, t1.uom
                from `tabSales BOM Item Y` t1, `tabSales BOM` t2
                where t2.new_item_code=%s and t1.parent = t2.name""", item_code, as_dict=1)

def get_packing_item_y_details(item):
...
def update_packing_list_y_item(obj, packing_item_code, qty, warehouse, line, packing_list_idx):
	bin = get_bin_qty(packing_item_code, warehouse)
	item = get_packing_item_y_details(packing_item_code)

	# check if exists
	exists = 0
	for d in obj.get("packing_details_y"):
		if d.parent_item == line.item_code and d.item_code == packing_item_code and d.parent_detail_docname == line.name:
			pi, exists = d, 1
			break

	if not exists:
		pi = obj.append('packing_details_y', {})
...
def make_packing_list_y(obj, item_table_fieldname):
	...
	for d in obj.get(item_table_fieldname):
		if frappe.db.get_value("Sales BOM", {"new_item_code": d.item_code}):
			for i in get_sales_bom_y_items(d.item_code):
				update_packing_list_y_item(obj, i['item_code'], flt(i['qty'])*flt(d.qty),
					d.warehouse, d, packing_list_idx)

			if [d.item_code, d.name] not in parent_items:
				parent_items.append([d.item_code, d.name])

	cleanup_packing_list_y(obj, parent_items)

def cleanup_packing_list_y(obj, parent_items):
	...
	packing_details_y = obj.get("packing_details_y")
	obj.set("packing_details_y", [])
	for d in packing_details_y:
		if d not in delete_list:
			obj.append("packing_details_y", d)
