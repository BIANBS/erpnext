...
class PackedItemZ(Document):
	pass

def get_sales_bom_z_items(item_code):
        return frappe.db.sql("""select t1.item_code, t1.qty, t1.uom
                from `tabSales BOM Item Z` t1, `tabSales BOM` t2
                where t2.new_item_code=%s and t1.parent = t2.name""", item_code, as_dict=1)

def get_packing_item_z_details(item):
...
def update_packing_list_z_item(obj, packing_item_code, qty, warehouse, line, packing_list_idx):
	bin = get_bin_qty(packing_item_code, warehouse)
	item = get_packing_item_z_details(packing_item_code)

	# check if exists
	exists = 0
	for d in obj.get("packing_details_z"):
		if d.parent_item == line.item_code and d.item_code == packing_item_code and d.parent_detail_docname == line.name:
			pi, exists = d, 1
			break

	if not exists:
		pi = obj.append('packing_details_z', {})
...
def make_packing_list_z(obj, item_table_fieldname):
	...
	cleanup_packing_list_z(obj, parent_items)

def cleanup_packing_list_z(obj, parent_items):
	...
	packing_details_z = obj.get("packing_details_z")
	obj.set("packing_details_z", [])
	for d in packing_details_z:
		if d not in delete_list:
			obj.append("packing_details_z", d)
